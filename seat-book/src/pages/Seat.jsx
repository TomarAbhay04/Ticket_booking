            ...
            seatsRows.push(
                <div key={row} className="flex justify-center mb-1">
                    ...
                    {rowSeats.map(seat => (
                        <button
                            key={seat._id}
                            className={`seat ${seat.available ? 'bg-green-500 border-solid border-green-500' : 'bg-gray-400'} ${selectedSeats.includes(seat._id) ? 'bg-red-500' : ''} ${hoveredSeat === seat._id ? 'bg-red-500' : '' } w-7 h-7 mb-1 mx-2 p-2 rounded-md flex items-center justify-center text-xs text-green-800 `}
